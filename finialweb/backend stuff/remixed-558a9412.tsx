import { useState } from "react";

const LOGO_SRC = "data:image/jpeg;base64,";

const C = {
  bg:            "#F4F6F9",
  surface:       "#FFFFFF",
  card:          "#FFFFFF",
  border:        "#DDE3ED",
  navy:          "#1E2D6B",
  navyDim:       "#E8ECF7",
  gray:          "#6B7A99",
  grayLight:     "#8A96A8",
  green:         "#1A7A4A",
  greenBg:       "#E8F5EE",
  red:           "#C0392B",
  redBg:         "#FDECEA",
  orange:        "#B7600A",
  orangeBg:      "#FEF3E2",
  blue:          "#1A5FAB",
  blueBg:        "#E6F0FB",
  textPrimary:   "#1A2340",
  textSecondary: "#5A6A8A",
  textMuted:     "#9AAABF",
};

const serviceTypes = [
  { value: "صيانة داخل الضمان", warranty: true,  visitFee: false },
  { value: "صيانة خارج الضمان", warranty: false, visitFee: true  },
];

function Card({ children, style = {} }) {
  return (
    <div style={{
      background: C.card, border: `1px solid ${C.border}`,
      borderRadius: 14, padding: 20,
      boxShadow: "0 1px 6px rgba(30,45,107,.07)", ...style,
    }}>{children}</div>
  );
}

function Btn({ children, color = C.navy, textColor = "#fff", outline, onClick, style = {}, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      padding: "11px 20px", borderRadius: 10, fontFamily: "inherit",
      fontWeight: 700, fontSize: 14, cursor: disabled ? "not-allowed" : "pointer",
      border: outline ? `1.5px solid ${color}` : "none",
      background: outline ? "transparent" : color,
      color: outline ? color : textColor,
      opacity: disabled ? .5 : 1, ...style,
    }}>{children}</button>
  );
}

function FieldLabel({ children }) {
  return <label style={{ color: C.textSecondary, fontSize: 13, display: "block", marginBottom: 6, fontWeight: 600 }}>{children}</label>;
}

const inputStyle = {
  width: "100%", padding: "11px 14px", borderRadius: 10,
  border: `1.5px solid ${C.border}`, background: C.bg,
  color: C.textPrimary, fontSize: 14, fontFamily: "inherit",
  outline: "none", boxSizing: "border-box",
};

export default function App() {
  const [tab, setTab]               = useState("new");
  const [form, setForm]             = useState({ client: "", phone: "", address: "", type: serviceTypes[0].value, desc: "", contractImage: null });
  const [submitted, setSubmitted]   = useState(false);
  const [trackPhone, setTrackPhone] = useState("");
  const [tracked, setTracked]       = useState([]);
  const [searched, setSearched]     = useState(false);

  const selectedService = serviceTypes.find(s => s.value === form.type) || serviceTypes[0];

  // بيانات تجريبية للتتبع
  const demoRequests = [
    { id: "REQ-001", phone: "0501234567", type: "صيانة داخل الضمان", desc: "الباب يصدر صوت عالي عند الفتح", status: "assigned", assignedTo: "فني: أحمد سالم", otp: "8472", pendingReason: null },
    { id: "REQ-002", phone: "0501234567", type: "صيانة خارج الضمان", desc: "صيانة وقائية", status: "closed", assignedTo: "فني: علي خالد", otp: "5521", pendingReason: null },
  ];

  const handleSubmit = () => {
    if (!form.client || !form.phone || !form.address || !form.desc) return;
    if (selectedService.warranty && !form.contractImage) return;
    setSubmitted(true);
    setTimeout(() => {
      setSubmitted(false);
      setForm({ client: "", phone: "", address: "", type: serviceTypes[0].value, desc: "", contractImage: null });
    }, 4000);
  };

  const handleTrack = () => {
    setTracked(demoRequests.filter(r => r.phone === trackPhone.trim()));
    setSearched(true);
  };

  const statusConfig = {
    pending:   { label: "قيد الانتظار", color: C.orange, bg: C.orangeBg, icon: "⏳" },
    assigned:  { label: "مُسند للفني",  color: C.blue,   bg: C.blueBg,   icon: "🔧" },
    suspended: { label: "معلق",         color: C.red,    bg: C.redBg,    icon: "⏸️" },
    closed:    { label: "مُغلق",        color: C.green,  bg: C.greenBg,  icon: "✅" },
  };

  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', 'Tajawal', 'Cairo', Arial, sans-serif", direction: "rtl" }}>

      {/* شريط تجريبي */}
      <div style={{ background: C.orange, color: "#fff", textAlign: "center", padding: "8px", fontSize: 13, fontWeight: 700 }}>
        🚧 نسخة تجريبية — للمراجعة الداخلية فقط
      </div>

      {/* الهيدر */}
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "0 24px", boxShadow: "0 2px 8px rgba(30,45,107,.07)" }}>
        <div style={{ maxWidth: 640, margin: "0 auto", display: "flex", alignItems: "center", padding: "10px 0", gap: 12 }}>
          <div style={{ width: 42, height: 42, borderRadius: 10, overflow: "hidden", background: "#fff", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <img src={LOGO_SRC} alt="logo" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
          </div>
          <div>
            <div style={{ color: C.navy, fontWeight: 900, fontSize: 16 }}>مدار راندا</div>
            <div style={{ color: C.textMuted, fontSize: 11 }}>بوابة طلبات الصيانة</div>
          </div>
        </div>
      </div>

      {/* المحتوى */}
      <div style={{ maxWidth: 640, margin: "0 auto", padding: 24 }}>

        {/* Tabs */}
        <div style={{ display: "flex", background: C.navyDim, borderRadius: 12, padding: 4, marginBottom: 24, gap: 4 }}>
          {[{ key: "new", label: "➕ طلب جديد" }, { key: "track", label: "🔍 تتبع طلباتي" }].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              flex: 1, padding: "11px", borderRadius: 9, border: "none", fontFamily: "inherit",
              background: tab === t.key ? C.navy : "transparent",
              color: tab === t.key ? "#fff" : C.textSecondary,
              fontWeight: 700, fontSize: 14, cursor: "pointer",
            }}>{t.label}</button>
          ))}
        </div>

        {tab === "new" ? (
          submitted ? (
            <Card style={{ textAlign: "center", padding: "52px 24px" }}>
              <div style={{ fontSize: 56, marginBottom: 16 }}>✅</div>
              <div style={{ color: C.green, fontSize: 22, fontWeight: 800, marginBottom: 10 }}>تم إرسال طلبك بنجاح!</div>
              <div style={{ color: C.textSecondary, fontSize: 15, lineHeight: 2 }}>نعمل على مراجعة الطلب وإرسال موعدك</div>
            </Card>
          ) : (
            <Card>
              <h3 style={{ color: C.textPrimary, margin: "0 0 22px", fontSize: 17, fontWeight: 800 }}>📝 بيانات طلب الصيانة</h3>

              {[
                { key: "client",  label: "الاسم الكامل", ph: "محمد العمري",     type: "text" },
                { key: "phone",   label: "رقم الجوال",   ph: "05XXXXXXXX",      type: "tel"  },
                { key: "address", label: "العنوان",       ph: "الحي، المدينة",  type: "text" },
              ].map(f => (
                <div key={f.key} style={{ marginBottom: 16 }}>
                  <FieldLabel>{f.label}</FieldLabel>
                  <input type={f.type} placeholder={f.ph} value={form[f.key]}
                    onChange={e => setForm({ ...form, [f.key]: e.target.value })}
                    style={inputStyle} />
                </div>
              ))}

              {/* نوع الخدمة */}
              <div style={{ marginBottom: 16 }}>
                <FieldLabel>نوع الخدمة</FieldLabel>
                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value, contractImage: null })}
                  style={{ ...inputStyle, appearance: "auto" }}>
                  {serviceTypes.map(t => <option key={t.value} value={t.value}>{t.value}</option>)}
                </select>
              </div>

              {/* تنبيه رسوم الزيارة */}
              {selectedService.visitFee && (
                <div style={{ marginBottom: 16, padding: "14px 16px", background: C.orangeBg, border: `1px solid ${C.orange}44`, borderRadius: 10, display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <span style={{ fontSize: 22 }}>💰</span>
                  <div>
                    <div style={{ color: C.orange, fontWeight: 700, fontSize: 14 }}>تنبيه: رسوم الزيارة</div>
                    <div style={{ color: C.textSecondary, fontSize: 13, marginTop: 4 }}>
                      رسوم الزيارة لهذه الخدمة <strong style={{ color: C.textPrimary }}>75 ريال</strong> تُحسب عند إرسال الفني
                    </div>
                  </div>
                </div>
              )}

              {/* رفع عقد الضمان */}
              {selectedService.warranty && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ padding: "12px 16px", background: C.blueBg, border: `1px solid ${C.blue}44`, borderRadius: 10, display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 12 }}>
                    <span style={{ fontSize: 22 }}>📄</span>
                    <div>
                      <div style={{ color: C.blue, fontWeight: 700, fontSize: 14 }}>مطلوب: صورة عقد الضمان</div>
                      <div style={{ color: C.textSecondary, fontSize: 13, marginTop: 4 }}>للتأكد من أن الطلب داخل فترة الضمان، يرجى إرفاق صورة من العقد</div>
                    </div>
                  </div>
                  <FieldLabel>صورة العقد <span style={{ color: C.red }}>*</span></FieldLabel>
                  <div
                    style={{ border: `2px dashed ${form.contractImage ? C.green : C.border}`, borderRadius: 10, padding: "20px", textAlign: "center", background: form.contractImage ? C.greenBg : C.bg, cursor: "pointer" }}
                    onClick={() => document.getElementById("contractInput").click()}
                  >
                    {form.contractImage ? (
                      <div>
                        <div style={{ fontSize: 30, marginBottom: 8 }}>✅</div>
                        <div style={{ color: C.green, fontWeight: 700, fontSize: 13 }}>{form.contractImage.name}</div>
                        <div style={{ color: C.textMuted, fontSize: 12, marginTop: 4 }}>اضغط لتغيير الصورة</div>
                      </div>
                    ) : (
                      <div>
                        <div style={{ fontSize: 30, marginBottom: 8 }}>📎</div>
                        <div style={{ color: C.textSecondary, fontSize: 14, fontWeight: 600 }}>اضغط لرفع صورة العقد</div>
                        <div style={{ color: C.textMuted, fontSize: 12, marginTop: 4 }}>JPG, PNG, PDF</div>
                      </div>
                    )}
                  </div>
                  <input id="contractInput" type="file" accept="image/*,.pdf" style={{ display: "none" }}
                    onChange={e => setForm({ ...form, contractImage: e.target.files[0] || null })} />
                </div>
              )}

              {/* وصف المشكلة */}
              <div style={{ marginBottom: 22 }}>
                <FieldLabel>وصف المشكلة</FieldLabel>
                <textarea placeholder="اشرح المشكلة بالتفصيل..." value={form.desc}
                  onChange={e => setForm({ ...form, desc: e.target.value })} rows={4}
                  style={{ ...inputStyle, resize: "vertical" }} />
              </div>

              {selectedService.warranty && !form.contractImage && form.client && (
                <div style={{ marginBottom: 12, color: C.red, fontSize: 13, textAlign: "center" }}>⚠️ يرجى رفع صورة العقد أولاً</div>
              )}

              <Btn onClick={handleSubmit} style={{ width: "100%", padding: 14, fontSize: 16 }}>
                إرسال الطلب 🚀
              </Btn>
            </Card>
          )
        ) : (
          <div>
            <Card style={{ marginBottom: 16 }}>
              <FieldLabel>ادخل رقم جوالك لمتابعة طلباتك</FieldLabel>
              <div style={{ display: "flex", gap: 10 }}>
                <input type="tel" placeholder="05XXXXXXXX" value={trackPhone}
                  onChange={e => setTrackPhone(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && handleTrack()}
                  style={{ ...inputStyle, flex: 1 }} />
                <Btn onClick={handleTrack} style={{ padding: "11px 20px" }}>بحث</Btn>
              </div>
              <div style={{ color: C.textMuted, fontSize: 12, marginTop: 8 }}>
                💡 للتجربة ادخل: 0501234567
              </div>
            </Card>

            {tracked.map(r => (
              <Card key={r.id} style={{ marginBottom: 12, borderRight: `4px solid ${statusConfig[r.status].color}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <span style={{ color: C.navy, fontWeight: 700, fontSize: 13 }}>{r.id}</span>
                    <div style={{ color: C.textPrimary, fontWeight: 700, marginTop: 4, fontSize: 15 }}>{r.type}</div>
                  </div>
                  <span style={{ background: statusConfig[r.status].bg, color: statusConfig[r.status].color, border: `1px solid ${statusConfig[r.status].color}55`, borderRadius: 20, padding: "4px 12px", fontSize: 12, fontWeight: 700, whiteSpace: "nowrap" }}>
                    {statusConfig[r.status].icon} {statusConfig[r.status].label}
                  </span>
                </div>
                <div style={{ color: C.textSecondary, fontSize: 13, marginBottom: 10 }}>{r.desc}</div>
                {r.assignedTo && <div style={{ padding: "8px 12px", background: C.blueBg, borderRadius: 8, color: C.blue, fontSize: 13, marginBottom: 8 }}>🔧 {r.assignedTo}</div>}
                {r.status === "assigned" && r.otp && (
                  <div style={{ padding: "14px 18px", background: C.navyDim, borderRadius: 10, border: `1.5px dashed ${C.navy}` }}>
                    <div style={{ color: C.textSecondary, fontSize: 12, marginBottom: 6 }}>رمز إغلاق الطلب (OTP) — أعطه للفني بعد الانتهاء فقط</div>
                    <div style={{ color: C.navy, fontWeight: 900, fontSize: 34, letterSpacing: 12, fontFamily: "monospace" }}>{r.otp}</div>
                  </div>
                )}
                {r.status === "closed" && (
                  <div style={{ padding: "8px 12px", background: C.greenBg, borderRadius: 8, color: C.green, fontSize: 13 }}>✅ تم إغلاق الطلب بنجاح</div>
                )}
              </Card>
            ))}

            {searched && tracked.length === 0 && (
              <div style={{ textAlign: "center", color: C.textMuted, padding: 48, background: C.surface, borderRadius: 14, border: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 36, marginBottom: 12 }}>🔍</div>
                لا يوجد طلبات لهذا الرقم
              </div>
            )}
          </div>
        )}
      </div>

      {/* Footer */}
      <div style={{ textAlign: "center", padding: "24px", color: C.textMuted, fontSize: 12, borderTop: `1px solid ${C.border}`, marginTop: 20 }}>
        © 2026 مدار راندا — جميع الحقوق محفوظة
      </div>
    </div>
  );
}
