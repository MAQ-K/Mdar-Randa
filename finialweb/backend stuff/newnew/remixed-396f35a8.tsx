...
// لوحة الفني
// ═══════════════════════════════════════════
function TechnicianView({ user, requests, groups, onLogout, onUpdateRequest }) {
 ...
 const handleClose = () => {
 if (closingOtp === selectedRequest.otp) {
 onUpdateRequest(selectedRequest.id, { status: "closed" });
 setSelectedRequest({ ...selectedRequest, status: "closed" });
 setClosingOtp("");
 setOtpError("");
 } else {
 setOtpError("رمز الإغلاق غير صحيح");
 }
 };
...
<Btn onClick={handleClose} color={C.green}>إغلاق </Btn>
</div>
...
// لوحة المشرف
// ═══════════════════════════════════════════
function SupervisorView({ user, requests, setRequests, technicians, setTechnicians, groups, setGroups, onLogout }) {
 ...
 const newReq = { id: "REQ-" + String(Date.now()).slice(-6), ...form, status: "pending", assignedGroup: null, otp: null, createdAt: new Date().toLocaleString("ar-SA") };
 ...
 const assignToGroup = (reqId, groupName) => {
 const updated = requests.map(r => r.id === reqId ? { ...r, assignedGroup: groupName, status: "assigned" } : r);
 setRequests(updated);
 setSelectedRequest(updated.find(r => r.id === reqId));
...
<Btn onClick={handleNewRequest} style={{ width: "100%", padding: 14, fontSize: 16 }}>إنشاء الطلب </Btn>
</Card>
))}

 {tab === "pending" && (
 selectedRequest && selectedRequest.status === "pending" ? (
 <RequestDetailCard request={selectedRequest} groups={groups} isAssigning={isAssigning} setIsAssigning={setIsAssigning} onAssign={assignToGroup} onBack={() => setSelectedRequest(null)} />
) : (
 <div>
...
function RequestDetailCard({ request, groups, isAssigning, setIsAssigning, onAssign, onBack }) {
 return (
 <Card>
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 20 }}>
<div>
...
export default function App() {
 ...
 return <AdminView user={currentUser} requests={requests} setRequests={setRequests} technicians={technicians} setTechnicians={setTechnicians} groups={groups} setGroups={setGroups} users={users} setUsers={setUsers} onLogout={logout} />;
 }
 if (currentUser.role === "supervisor") {
 return <SupervisorView user={currentUser} requests={requests} setRequests={setRequests} technicians={technicians} setTechnicians={setTechnicians} groups={groups} setGroups={setGroups} onLogout={logout} />;
 }
 return <TechnicianView user={currentUser} requests={requests} groups={groups} onUpdateRequest={updateRequest} onLogout={logout} />;
}
