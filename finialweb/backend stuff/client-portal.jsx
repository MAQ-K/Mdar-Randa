...
export default function App() {
  ...
  // ملاحظة: رفع صورة العقد (contractImage) غير مدعوم حالياً في الخادم
  // (server.js يستقبل JSON فقط)، لذلك لا يتم إرسالها ضمن الطلب بعد.
  ...
  const handleTrack = async () => {
    ...
    setTrackError("");
    try {
      const data = await apiRequest(`/requests/phone/${encodeURIComponent(trackPhone.trim())}`);
      setTracked(data);
      setSearched(true);
    } catch (e) {
        ...
        {tab === "new" ? (
          submitted ? (
            ...
            <Card>
              ...
              {submitError && (
                <div style={{ marginBottom: 12, color: C.red, fontSize: 13, textAlign: "center" }}>⚠️ {submitError}</div>
              )}

              <Btn onClick={handleSubmit} disabled={submitting} style={{ width: "100%", padding: 14, fontSize: 16 }}>
          ...
          <div>
            <Card style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <Btn onClick={handleTrack} disabled={tracking} style={{ padding: "11px 20px" }}>{tracking ? "..." : "بحث"}</Btn>
              </div>
              {trackError && (
                <div style={{ color: C.red, fontSize: 12, marginTop: 8 }}>⚠️ {trackError}</div>
              )}
            </Card>
            ...
            {tracked.map(r => (
              <Card key={r.id} style={{ marginBottom: 12, borderRight: `4px solid ${statusConfig[r.status].color}` }}>
                ...
                {r.assignedGroup && <div style={{ padding: "8px 12px", background: C.blueBg, borderRadius: 8, color: C.blue, fontSize: 13, marginBottom: 8 }}>🔧 فريق: {r.assignedGroup}</div>}
